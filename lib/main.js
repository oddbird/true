...
var REG_EDGE_QUOTES = /(^['"]|['"]$)/g;
var REG_COMMENT = /\/\*[\s\S]*?\*\/|([^\\:]|^)\/\/.*$/gm;
var REG_ASSERT_ERROR = /@include\s+assert-error[^-\(]*\(/;
var REG_ASSERT_ERROR_EXEC = /@include\s+_assert-error-execute[^-\(]*\(([^\)]*)\)/;
...
var runSass = function(sassOptions, trueOptions) {
  ...
  // remove comments in case someone commented an error assertion
  sassOpts.data = sassOpts.data.replace(REG_COMMENT, '$1');
  parseErrorAssertions(sass, sassOpts);
...
// By default the `assert-error` mixin is a no-op. In order to test errors,
// we replace each assertion **one at a time** with _assert-error-execute.
// We then call `sass.renderSync` inside of a try/catch block.
// If no error is thrown, or the emitted message is not what is expected,
// then we render _assert-error-fail with the appropriate message. If an
// error is thrown (and the message matches), the we render _assert-error-success.
// NOTE: this mutates sassOpts.data in place
var parseErrorAssertions = function(sass, sassOpts) {
  // We must first compile the original file to ensure there are no errors.
  // If there are errors, this will terminate the process and jest will report it.
  sass.renderSync(sassOpts);

  // for each assertion, replace with `_assert-error-execute` mixin
  while (REG_ASSERT_ERROR.test(sassOpts.data)) {
    sassOpts.data = sassOpts.data.replace(REG_ASSERT_ERROR,
      '@include _assert-error-execute('
    );

    try {
      sass.renderSync(sassOpts);

      // error did not occur, report as failure
      sassOpts.data = sassOpts.data.replace(REG_ASSERT_ERROR_EXEC,
        '@include _assert-error-fail("Expected an error to be thrown", "No error thrown")'
      );
    } catch (ex) {
      // an error occured, compare messages if one was provided
      sassOpts.data = sassOpts.data.replace(REG_ASSERT_ERROR_EXEC, function(match, message) {
        // use case-insensitive indexOf test to allow loose testing
        var msg = message.trim().replace(REG_EDGE_QUOTES, '').toLowerCase();
        if (msg && ex.message.toLowerCase().indexOf(msg) === -1) {
          return '@include _assert-error-fail("' + ex.message + '", ' + message + ')';
        }

        return '@include _assert-error-success(' + message + ')';
      });
    }
  }
};
