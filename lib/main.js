...
var parseModule = function (rule, ctx) {
  ...
  if (rule.type === 'comment') {
    ...
    if (text.substring(0, MODULE_TOKEN.length) === MODULE_TOKEN) {
      finishCurrentModule(ctx);
      ctx.currentModule = { module: text.substring(MODULE_TOKEN.length), tests: [] };
      return parseTest;
    }
    if (text.substring(0, SUMMARY_TOKEN.length) === SUMMARY_TOKEN) {
      return ignoreUntilEndSummary;
    }
    throw parseError('Unexpected comment "' + text + '"', 'module header', rule.position);
  }
...
var ignoreUntilEndSummary = function (rule, ctx) {
  if (rule.type === 'comment') {
    var text = rule.comment.trim();
    if (text.substring(0, END_SUMMARY_TOKEN.length) === END_SUMMARY_TOKEN) {
      return parseModule;
    }
    return ignoreUntilEndSummary;
  }
...
var parseTest = function (rule, ctx) {
  if (rule.type === 'comment') {
    ...
    if (text.substring(0, TEST_TOKEN.length) === TEST_TOKEN) {
...
var parseAssertion = function (rule, ctx) {
  if (rule.type === 'comment') {
    ...
    if (text.substring(0, PASS_TOKEN.length) === PASS_TOKEN) {
      ...
      return parseAssertion;
    } else if (text.substring(0, FAIL_TOKEN.length) === FAIL_TOKEN) {
      finishCurrentAssertion(ctx);
    ...
    } else if (text.substring(0, ASSERT_TOKEN.length) === ASSERT_TOKEN) {
      finishCurrentAssertion(ctx);
      ctx.currentAssertion = {
...
var parseFailureDetail = function (rule, ctx) {
  if (rule.type === 'comment') {
    var text = rule.comment.trim();
    if (text.substring(0, FAILURE_DETAIL_TOKEN.length) === FAILURE_DETAIL_TOKEN) {
