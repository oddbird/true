...
var contains = function (output, expected) {
  ...
  var results = expectedBlocks.map(function (block) {
    ...
    if (outputBlock) {
      ...
      var expectedProperties = block.output.split('\n').filter(function (block) {
        return block !== ' {' && block !== '}';
      });
...
var dealWithAnnoyingMediaQueries = function (rawCSS) {
  ...
  var matchMediaQueryBlock = /@[a-zA-Z0-9:()\s-]+{([a-zA-Z0-9:()\s-;\._\\n{}]+)(?!}\\n})/g;

  var mediaqueries = rawCSS.match(matchMediaQuery);
  var rawCSSSansMediaQueries = rawCSS.replace(matchMediaQuery,'').replace(matchMediaQueryBlock,'').replace(/^{/,'');
  var matches = matchMediaQueryBlock.exec(rawCSS);
  var i = 0;
  var mediaQueryBasedSelectors = [];
  while (matches !== null) {
    // This is necessary to avoid infinite loops with zero-width matches
    if (matches.index === matchMediaQueryBlock.lastIndex) {
        matchMediaQueryBlock.lastIndex++;
    }
    ...
    i++;
    matches = matchMediaQueryBlock.exec(rawCSS);
  }
